...
begin
   ...
   Put_Line ("SPI ready, starting read...");
   ...
   begin
      ...
      begin
         ...
         CS_High;
         Put_Line ("JEDEC: " &
                  Integer'Image (Integer (ID (1))) & " " &
                  Integer'Image (Integer (ID (2))) & " " &
                  Integer'Image (Integer (ID (3))));
      end;
      declare
      ...
      begin
         ...
         Transfer (Data,  Data);
         CS_High;
         --  Put_Line ("Direct read bytes 1-4: " &
         --           Integer'Image (Integer (Data (1))) & " " &
         --           Integer'Image (Integer (Data (2))) & " " &
         --           Integer'Image (Integer (Data (3))) & " " &
         --           Integer'Image (Integer (Data (4))));
      end;
      loop
         Read_Page (Read_Addr);

         -- Debug: always print raw bytes of page 0
         --  if Page_Idx = 0 then
         --     Put_Line ("Page 0 raw bytes 250-256:");
         --     for I in 250 .. 256 loop
         --        declare
         --           B : constant HAL.UInt8 := Page_Buf (I);
         --        begin
         --           Put_Line ("  [" & Integer'Image (I) & "] = " & Integer'Image (Integer (B)));
         --        end;
         --     end loop;
         --  end if;
         
         --  Put_Line ("Page 0 raw bytes 1-4:");
         --  for I in 1 .. 4 loop
         --     declare
         --        B : constant HAL.UInt8 := Page_Buf (I);
         --     begin
         ...
         if Page_Buf (253) = 16#FF# and then
            ...
            Page_Buf (256) = 16#FF#
         then
            Put_Line ("End of data at page" & Integer'Image (Page_Idx));
            exit;
         end if;
         ...
         if Page_Idx >= MAX_PAGE then
            Put_Line ("Reached end of flash (64K pages) - stopping");
            exit;
         end if;
